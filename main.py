...
def handler(event=None, context=None):
    ...
    # TODO: still need to 1) ingest the SNS event, and get the URL / other info from that, and 2) publish to Dynamoo
    # depending on solve percentage / existence of something in lambda, maybe don't use results.
...
def process_scraped_minesweeper_game(result_block_text, difficulty_selector_html, username):
    ...
    solve_percentage_value = 100.0 if board_solved else float(completed_3bv_value)/float(board_3bv_value)
...
dotenv_path = join(dirname(__file__), '.env')
load_dotenv(dotenv_path)
scrape = scrape_minesweeper_online_game("https://minesweeper.online/game/1986919801")
result = process_scraped_minesweeper_game(scrape[0], scrape[1], os.environ['MINESWEEPER_USERNAME'])
print(result)